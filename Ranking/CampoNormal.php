<?php
	...
	$BuscPaginasRanking->execute(array(0));
	$QTDPaginas = $BuscPaginasRanking->fetch(PDO::FETCH_NUM);
	$totalPaginasRanking = ceil($QTDPaginas[0]/3);
	if(isset($_GET['pagRankingNormal']))
	{
		$pgRankNormal = $_GET['pagRankingNormal'];
		$inicioRanking = ($pgRankNormal - 1) * 3;
	}
	else
	{
		isset($_GET['pagRankingNormal']);
		$pgRankNormal = 1;
	}
	
?>
<div class="row">
    <div class="card-panel blue darken-4 white-text" style="margin-top:8px;"><center>Ranking - Campo Normal</center></div>
    <table class="responsive-table striped centered">
        ...
        <tbody>
        	<?php
				...
				$BuscDadosRanking->execute(array(0));
				while($BDK = $BuscDadosRanking->fetch(PDO::FETCH_NUM))
				{
					...
					echo "
							<tr>
								<td>".$Classificacao."</td>
								<td><img class='responsive-img' src='image/times/".$BDT[1]."' width='100px'></td>
								<td>".$BDT[0]."</td>
    ...
    <ul class="pagination right">
    	<?php
        	if($totalPaginasRanking > 0)
			{
				if($pgRankNormal > 1)
				{
					 echo "<li><a href='index.php?pag=Ranking&pagRankingNormal=".($pgRankNormal - 1)."'><i class='material-icons'>chevron_left</i></a></li>";
				}
				else
				{
				...
				for($i = 1; $i <= $totalPaginasRanking; $i++)
				{
					...
					{
						echo "<li><a href='index.php?pag=Ranking&pagRankingNormal=".$i."'>".$i."</a></li>";
					}
				}
				
				if($pgRankNormal == $totalPaginasRanking)
				{
					echo "<li class='desabled'><i class='material-icons'>chevron_right</i></li>";
				}
				else
				{
					echo "<li><a href='index.php?pag=Ranking&pagRankingNormal=".($pgRankNormal + 1)."'><i class='material-icons'>chevron_right</i></a></li>";
				}
			}
			else
			{
